              ...
              <tr key={`${entry.label}-${idx}`} className="group">
                ...
                {showTarget && (
                  <td className="py-2 md:py-3 px-3 md:px-5 text-right">
                    <input type="text" inputMode="numeric" pattern="[0-9.]*" value={`${idx}-target` in localNums ? localNums[`${idx}-target`] : (entry.target || '')} onChange={(e) => { const raw = e.target.value.replace(/[^0-9.]/g, ''); setLocalNums(prev => ({ ...prev, [`${idx}-target`]: raw })); onChangeWrapper([...path, idx.toString()], 'target', raw); }} onBlur={() => { const raw = localNums[`${idx}-target`]; if (raw !== undefined) onChangeWrapper([...path, idx.toString()], 'target', raw); }} className="w-20 md:w-28 bg-[#eef1f1] border border-[#45d0d0]/20 rounded-lg px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm text-right outline-none focus:border-[#f78121] text-[#595b61] font-bold" />
                  </td>
                )}
                <td className="py-2 md:py-3 px-3 md:px-5 text-right">
                  <input type="text" inputMode="numeric" pattern="[0-9.]*" value={`${idx}-actual` in localNums ? localNums[`${idx}-actual`] : (entry.actual ?? entry.value ?? '')} onChange={(e) => { const raw = e.target.value.replace(/[^0-9.]/g, ''); setLocalNums(prev => ({ ...prev, [`${idx}-actual`]: raw })); onChangeWrapper([...path, idx.toString()], entry.value !== undefined ? 'value' : 'actual', raw); }} onBlur={() => { const raw = localNums[`${idx}-actual`]; if (raw !== undefined) onChangeWrapper([...path, idx.toString()], entry.value !== undefined ? 'value' : 'actual', raw); }} className="w-20 md:w-28 bg-[#eef1f1] border border-[#45d0d0]/20 rounded-lg px-2 md:px-3 py-1.5 md:py-2 text-xs md:text-sm text-right outline-none focus:border-[#f78121] text-[#595b61] font-bold" />
                </td>
                <td className="py-2 md:py-3 px-3 md:px-5 text-right">
